(() => {
  if (globalThis.__temuGptListingV2Loaded) {
    return;
  }
  globalThis.__temuGptListingV2Loaded = true;
  ...
  async function runPlan(message) {
    ...
    await setPromptAndSend(message.prompt, message.timeoutMs || 240000);
  ...
  async function runExecute(message) {
    ...
    await setPromptAndSend(message.prompt, message.timeoutMs || 420000);
  ...
  async function attachImage(filePayload) {
    ...
    await waitForAttachmentProcessingToFinish(120000);
  ...
  async function waitForAttachmentProcessingToFinish(timeoutMs = 120000) {
    await waitFor(() => !isAttachmentProcessing(), timeoutMs, "GPT image attachment is still processing");
  }
  ...
  async function setPromptAndSend(prompt, timeoutMs = 30000) {
    const composer = await waitForComposer();
    focusAndSetText(composer, prompt);
    await delay(250);
    const sendButton = await waitForSendButton(timeoutMs);
    sendButton.click();
  }
  ...
  async function waitForSendButton(timeoutMs = 30000) {
    return await waitFor(() => {
      if (isAttachmentProcessing()) return null;
