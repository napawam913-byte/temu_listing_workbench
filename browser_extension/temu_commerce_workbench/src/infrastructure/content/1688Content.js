(function () {
  if (globalThis.__temuWorkbench1688ContentLoaded) return;
  globalThis.__temuWorkbench1688ContentLoaded = true;
  ...
  function parseMoney(text) {
    ...
    const currencyMatch = normalized.match(/(?:[￥¥]|rmb|cny)\s*([0-9]+(?:\.[0-9]+)?)/i);
  ...
  function getInlineContextScriptText() {
    return Array.from(document.scripts || [])
      ...
      .filter((text) => text.includes("window.context=") || text.includes('"tradeModel"') || text.includes('"unitWeight"'))
      .join("\n");
  }
  ...
  function findWeightFromWeightHeaderText(sourceText) {
    ...
    const patterns = [
      /(?:\u5546\u54c1\u4ef6\u91cd\u5c3a[\s\S]{0,80}?)?\u91cd\u91cf\s*[\(（]?\s*(kg|g|\u5343\u514b|\u516c\u65a4|\u514b)\s*[\)）]?[^\d]{0,30}([0-9]+(?:\.[0-9]+)?)/i,
  ...
  function findGoodsPrice() {
    ...
    return extractValueByPatterns(getInlineContextScriptText(), [
      /"maxPrice":"([0-9]+(?:\.[0-9]+)?)"/,
  ...
  function extractProductData() {
    const goodsPrice = Number(findGoodsPrice() || 0);
    const shippingFee = Number(findShippingFee() || 0);
    const weight = Number(findWeight() || 0);
    return {
      url: window.location.href,
      goodsPrice,
      shippingFee,
      unitPrice: goodsPrice > 0 ? Number((goodsPrice + shippingFee).toFixed(2)) : "",
      weight: weight > 0 ? weight : ""
    };
  }

  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type !== "extract-1688-product-data") return false;
